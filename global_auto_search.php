...
{            
    $sql ="SELECT DISTINCT ofd.ORACLE_ORDER FROM order_form_details as ofd, order_products as op, xxdkapps_unsegregated_products as up, xxdkapps_customer_master as cm WHERE ofd.CUSTOMER_NUMBER= " . $cust_id." and ofd.ORACLE_ORDER = '".$_REQUEST['auto_search_value']."' and op.PRODUCT_CODE = up.PRODUCT_CODE and op.ORDER_ID = ofd.ID and ofd.SHIP_TO = cm.SITE_USE_ID ORDER BY ofd.ORACLE_ORDER";
    $result= $con->data_select($sql);
...
{        
    $sql ="SELECT DISTINCT ofd.ORDER_WEB_ID FROM order_form_details as ofd, order_products as op, xxdkapps_unsegregated_products as up, xxdkapps_customer_master as cm WHERE ofd.CUSTOMER_NUMBER= " . $cust_id." and ofd.ORDER_WEB_ID = '".$_REQUEST['auto_search_value']."' and op.PRODUCT_CODE = up.PRODUCT_CODE and op.ORDER_ID = ofd.ID and ofd.SHIP_TO = cm.SITE_USE_ID ORDER BY ofd.ORDER_WEB_ID";
    $result= $con->data_select($sql);
...
if(isset($_REQUEST['search_category']) && ($_REQUEST['search_category'] == "7") )
{	
    ...
    if($result == 'no'){
		...
		foreach ($result as $key => $value) {
	    	array_push($data, $result[$key]['PO_NUMBER']);
	    }
	}
	echo json_encode($data);  
    exit;    
